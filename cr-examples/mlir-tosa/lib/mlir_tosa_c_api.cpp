...
struct MLIRFunction_ {
    ...
    MLIRFunction_(MLIRContext *ctx, func::FuncOp op) : context(ctx), funcOp(op), builder(ctx) {
...
thread_local std::string lastError; // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
...
MLIRResultCode mlir_module_write_text(MLIRModuleHandle module, const char *filename) {
    if (module == nullptr || filename == nullptr) {
...
char *mlir_module_to_string(MLIRModuleHandle module) {
    ...
    char *result = static_cast<char *>(malloc(str.length() + 1)); // NOLINT(cppcoreguidelines-no-malloc)
    if (result != nullptr) {
...
MLIRTypeHandle mlir_type_create_tensor_ranked(MLIRContextHandle context, const int64_t *shape,
    ...
    if (context == nullptr || elementType == nullptr || (rank > 0 && shape == nullptr)) {
        setError("Invalid parameter");
        return nullptr;
    }
...
MLIRTypeHandle mlir_type_create_tensor_dynamic(MLIRContextHandle context, size_t rank,
    ...
    if (context == nullptr || elementType == nullptr) {
        setError("Invalid parameter");
        return nullptr;
    }
...
MLIRValueHandle mlir_tosa_add(MLIRFunctionHandle function, MLIRValueHandle lhs, MLIRValueHandle rhs,
    ...
    if (function == nullptr || lhs == nullptr || rhs == nullptr || resultType == nullptr) {
        setError("Invalid parameter");
        return nullptr;
    }
    ...
    auto addOp =
        function->builder.create<tosa::AddOp>(loc, resultType->type, lhs->value, rhs->value);
...
MLIRValueHandle mlir_tosa_sub(MLIRFunctionHandle function, MLIRValueHandle lhs, MLIRValueHandle rhs,
    ...
    auto loc = UnknownLoc::get(function->context);
    auto subOp =
        function->builder.create<tosa::SubOp>(loc, resultType->type, lhs->value, rhs->value);
...
MLIRValueHandle mlir_tosa_mul(MLIRFunctionHandle function, MLIRValueHandle lhs, MLIRValueHandle rhs,
    ...
    auto loc = UnknownLoc::get(function->context);
    auto mulOp =
        function->builder.create<tosa::MulOp>(loc, resultType->type, lhs->value, rhs->value, 0);
...
MLIRValueHandle mlir_tosa_negate(MLIRFunctionHandle function, MLIRValueHandle input,
                                 MLIRTypeHandle resultType) {
    if (function == nullptr || input == nullptr || resultType == nullptr) {
        setError("Invalid parameter");
        return nullptr;
    }
...
MLIRValueHandle mlir_tosa_reciprocal(MLIRFunctionHandle function, MLIRValueHandle input,
    ...
    auto loc = UnknownLoc::get(function->context);
    auto reciprocalOp =
        function->builder.create<tosa::ReciprocalOp>(loc, resultType->type, input->value);
...
MLIRValueHandle mlir_tosa_matmul(MLIRFunctionHandle function, MLIRValueHandle lhs,
    ...
    auto loc = UnknownLoc::get(function->context);
    auto matmulOp =
        function->builder.create<tosa::MatMulOp>(loc, resultType->type, lhs->value, rhs->value);
...
MLIRValueHandle mlir_tosa_relu(MLIRFunctionHandle function, MLIRValueHandle input,
    ...
    auto clampOp = function->builder.create<tosa::ClampOp>(loc, resultType->type, input->value, 0,
                                                           INT64_MAX, minFp, maxFp);
...
MLIRValueHandle mlir_tosa_conv2d(MLIRFunctionHandle function, MLIRValueHandle input,
                                 ...
                                 MLIRTypeHandle resultType) {
    if (function == nullptr || input == nullptr || weight == nullptr || bias == nullptr ||
        pad == nullptr || stride == nullptr || dilation == nullptr || resultType == nullptr) {
        setError("Invalid parameter");
        return nullptr;
    }
    ...
    if (biasType && biasType.getRank() > 1) { // NOLINT(readability-implicit-bool-conversion)
        ...
        auto reshapeOp =
            function->builder.create<tosa::ReshapeOp>(loc, bias1DType, biasValue, newShapeAttr);
        biasValue = reshapeOp.getResult();
    }

    auto conv2dOp =
        function->builder.create<tosa::Conv2DOp>(loc, resultType->type, input->value, weight->value,
                                                 biasValue, padAttr, strideAttr, dilationAttr);
...
MLIRValueHandle mlir_tosa_max_pool2d(MLIRFunctionHandle function, MLIRValueHandle input,
    ...
    if (function == nullptr || input == nullptr || kernel == nullptr || stride == nullptr ||
        pad == nullptr || resultType == nullptr) {
        setError("Invalid parameter");
        return nullptr;
    }
    ...
    auto maxPool2dOp = function->builder.create<tosa::MaxPool2dOp>(
        loc, resultType->type, input->value, kernelAttr, strideAttr, padAttr);
...
MLIRValueHandle mlir_tosa_avg_pool2d(MLIRFunctionHandle function, MLIRValueHandle input,
    ...
    auto avgPool2dOp = function->builder.create<tosa::AvgPool2dOp>(
        loc, resultType->type, input->value, kernelAttr, strideAttr, padAttr, accType);
...
MLIRValueHandle mlir_tosa_reshape(MLIRFunctionHandle function, MLIRValueHandle input,
                                  ...
                                  MLIRTypeHandle resultType) {
    if (function == nullptr || input == nullptr || newShape == nullptr || resultType == nullptr) {
        setError("Invalid parameter");
        return nullptr;
    }
    ...
    auto reshapeOp = function->builder.create<tosa::ReshapeOp>(loc, resultType->type, input->value,
                                                               newShapeAttr);
...
MLIRValueHandle mlir_tosa_reduce_sum(MLIRFunctionHandle function, MLIRValueHandle input,
    ...
    auto axisAttr = function->builder.getI32IntegerAttr(static_cast<int32_t>(axis));

    auto reduceSumOp =
        function->builder.create<tosa::ReduceSumOp>(loc, resultType->type, input->value, axisAttr);
...
MLIRValueHandle mlir_tosa_reduce_max(MLIRFunctionHandle function, MLIRValueHandle input,
    ...
    auto reduceMaxOp =
        function->builder.create<tosa::ReduceMaxOp>(loc, resultType->type, input->value, axisAttr);
...
MLIRValueHandle mlir_tosa_const_f32(MLIRFunctionHandle function, const float *data,
                                    ...
                                    MLIRTypeHandle resultType) {
    if (function == nullptr || data == nullptr || resultType == nullptr) {
        setError("Invalid parameter");
        return nullptr;
    }

    if (numDims > 0 && shape == nullptr) {
    ...
    if (!tensorType) { // NOLINT(readability-implicit-bool-conversion)
